...
~~現在~~(2020年12月)は以下を担当
2020年4月から自社Webサービスのスクラムマスター兼開発者。自身を含めた全メンバーがスクラム未経験、社内では初のスクラム開発を実施。スクラム開発の浸透のため読書会を実施。
VSCode, Remote Containerを用いて標準の開発環境を整備。マイクロサービス化を提案して各処理をAPI化して開発。

現在(2021年2月〜)、SREに転向、が、現在社内にSRE業務自体がほとんどなく、勉強しつつ何でも屋として案件の作業をしたり。
...
Github Actions|3ヶ月|3|基本的な実装なら可能。push時のテスト実行やSlack通知など。
DGX Station|1ヶ月|1|前任者がRancher等でクラスタ構築し、jobを投げれるようにしてあるものを引き継いだ。触るの怖い。
...
FastAPI|1ヶ月|2|社内用webチュートリアル作成に使用
...
--|--|--|--
Git|3年|4|
Github|1.5年|3|Github Actionsによる自動テストやSlack通知なども。
